...
# 2020年: 一人あたり賃金のヒートマップ
st.header("■ 2020年: 一人あたり賃金のヒートマップ")
...
df_pref_map['一人当たり賃金（相対値）'] = (
    (df_pref_map['一人当たり賃金（万円）'] - df_pref_map['一人当たり賃金（万円）'].min())) / ((df_pref_map['一人当たり賃金（万円）'].max() - df_pref_map['一人当たり賃金（万円）'].min())
    )
...
df_mean_buble = df_jp_ind[df_jp_ind['年齢'] != '年齢計']

fig = px.scatter(df_mean_buble,
                x="一人当たり賃金（万円）",
                y='年間賞与その他特別給与額（万円）',
                range_x=[150,700],
                range_y=[0,150],
...
st.text('本サイトは、RESAS（地域経済分析システム）提供のデータを加工して作成しました。')
